...
private:
	...
	// "wobbly marquee" look) and a thick colored outline for legibility over busy backgrounds - used
	// for both the everyday combo counter and the big tier-up banner rather than a single flat
	// DrawText call. WobblePhase offsets the per-letter sine so two simultaneous wobbly texts don't
	// bounce in lockstep. OutlineColor defaults to near-black but the combo number readout passes the
	// combo's own color with a white fill for extra "hit marker" contrast.
	...
	// Radiating "impact"/speed lines behind a combo readout or milestone banner - a handful of
	// strokes fanning out from Center, rotating slowly so the banner never looks static while it's up.
	void DrawStarburst(const FVector2D& Center, float InnerRadius, float OuterRadius, const FLinearColor& Color,
		int32 RayCount, float RotationOffset, float Thickness = 4.0f) const;
